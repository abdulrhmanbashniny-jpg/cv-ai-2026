import { LayoutDashboard, Inbox, Brain, Puzzle, FileText, LogOut, Bot, Cog, TrendingUp, FileUser, Package, Users, Megaphone, Bell, ShoppingCart } from "lucide-react";
import {
...
const items = [
  ...
  { title: "مركز قيادة الوكلاء", value: "aiCommand", icon: Brain },
  { title: "المحلل الذكي (CAIO)", value: "caio", icon: TrendingUp },
  { title: "الطلبات", value: "orders", icon: ShoppingCart },
  ...
  { title: "مركز التنبيهات", value: "notifications", icon: Bell },
            ...
            <SidebarMenu>
              {items.map((item) => (
                <SidebarMenuItem key={item.value}>
                  <SidebarMenuButton onClick={() => onTabChange(item.value)} isActive={activeTab === item.value} className="font-arabic gap-3">
